...
| `gatesprt` | the AZ promotion gate's sequential test (`train/gate_sprt.py`, via `train/test_gate_sprt.py`): hypotheses symmetric about 0.5, draws scored half/half, Wald bounds, verdict monotone and mirror-symmetric, the round cap's tie-break unbiased (score > 0.5, not 0.55), and simulated gates accepting a stronger candidate in fewer matches than the fixed panel it replaced, rejecting a weaker one, and coin-flipping an EQUAL one | any change to the promote/keep math — which silently changes which nets get promoted and has no other symptom. Stdlib-only, torch-free, engine-free, instant |
