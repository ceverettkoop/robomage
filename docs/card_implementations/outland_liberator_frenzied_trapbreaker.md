...
- **Own-turn spell count.** `Player::spells_cast_this_turn` is reset only at its OWN player's
  cleanup, so a player's instants cast on the opponent's turn would leak into their own-turn count.
  To isolate "spells the active player cast during their own turn" (which is what 731.2 needs),
  `advance_step` snapshots the active player's counter at the start of their turn
  (`Game::active_spells_at_turn_start`) and stores `current - snapshot` into
  `Game::prev_turn_active_spell_count` at cleanup (before the per-turn reset). The shared counter's
  "this turn" semantics other cards consume are left untouched.
