...
static void  draw_wrapped_text(const char *text, float x, float y,
                                float max_w, float font_size, Color color);
static void  draw_perm_chip(float sx, float sy, float sw, float sh,
                             const PermanentState *perm, Vector2 mouse);
static void  render_battlefield(float px, float py, float pw, float ph,
                                 const PermanentState *perms, int max_slots,
...
static void draw_perm_chip(float sx, float sy, float sw, float sh,
    ...
    if (perm->card_vocab_idx < 0) {
        ...
        DrawRectangleLinesEx((Rectangle){sx + 1, sy + 1, sw - 2, sh - 2},
                             0.5f, (Color){190, 190, 190, 60});
        return;
    }
    ...
    DrawRectanglePro((Rectangle){cx, cy, cw, ch},
                     (Vector2){cw * 0.5f, ch * 0.5f}, rot, bg);
    ...
    for (int i = 0; i < 4; i++)
        DrawLineEx(sc[i], sc[(i + 1) % 4], 1.5f, border_col);
    ...
    if (!perm->is_tapped) {
        if (sw >= 30.0f) {
            DrawTextEx(g_font, name, (Vector2){sx + 3.0f, sy + 3.0f},
                       font_sz, 1.0f, BLACK);
            if (perm->is_creature && sw >= 40.0f) {
                char pt[32];
                if (perm->damage > 0)
                    snprintf(pt, sizeof(pt), "%d/%d (%ddmg)",
                             perm->power, perm->toughness, perm->damage);
                else
                    snprintf(pt, sizeof(pt), "%d/%d",
                             perm->power, perm->toughness);
                DrawTextEx(g_font, pt,
                           (Vector2){sx + 3.0f, sy + sh - font_sz - 3.0f},
        ...
        if (ch >= 30.0f) {
            ...
            DrawTextPro(g_font, name,
                        (Vector2){cx - ch * 0.25f, cy},
                        (Vector2){name_w * 0.5f, font_sz * 0.5f},
            ...
            if (perm->is_creature) {
                ...
                DrawTextPro(g_font, pt,
                            (Vector2){cx + ch * 0.25f, cy},
                            (Vector2){pt_w * 0.5f, font_sz * 0.5f},
    ...
    bool hovered;
    if (!perm->is_tapped) {
        hovered = (mouse.x >= sx && mouse.x < sx + sw &&
...
static void render_battlefield(float px, float py, float pw, float ph,
    ...
    for (int d = active; d < display_slots; d++) {
        float sx = px + (float)d * slot_w;
        DrawRectangleLinesEx((Rectangle){sx + 1.0f, py + 1.0f, slot_w - 2.0f, ph - 2.0f},
                             0.5f, (Color){190, 190, 190, 60});
    }
}
...
static void render_gs(void) {
    ...
    // ── 1. Turn / Step bar ──────────────────────────────────────────────
    {
        ...
        snprintf(bar, sizeof(bar),
                 "Turn %d  |  %s  |  %s  |  Priority: %s",
                 gs->turn,
                 gui_step_name((int)gs->cur_step),
                 gs->is_active_player ? "Your turn" : "Opp turn",
                 gs->is_active_player ? "You" : "Opp");
    ...
    // ── 2. Opponent info bar ────────────────────────────────────────────
    {
        char bar[256];
        snprintf(bar, sizeof(bar),
                 "OPP  Life:%d  Poison:%d  W:%d U:%d B:%d R:%d G:%d C:%d  Hand:%d  Lib:%d",
                 gs->opponent.life, gs->opponent.poison_counters,
                 gs->opponent.mana[0], gs->opponent.mana[1], gs->opponent.mana[2],
                 gs->opponent.mana[3], gs->opponent.mana[4], gs->opponent.mana[5],
    ...
    // ── 3. Opponent hand (face-down) ────────────────────────────────────
    {
        ...
        for (int i = 0; i < n; i++) {
            ...
            DrawRectangle((int)cx_i, (int)(y + 4.0f), (int)card_w, (int)(hand_h - 8.0f),
                          (Color){75, 35, 115, 210});
            DrawRectangleLinesEx(
                (Rectangle){cx_i, y + 4.0f, card_w, hand_h - 8.0f},
                1.0f, (Color){40, 20, 75, 255});
        }
        y += hand_h + 4.0f;
    }
    ...
    {
        float bf_h = 175.0f;
        render_battlefield(px, y, pw, bf_h, gs->opp_permanents,
                           MAX_BATTLEFIELD_SLOTS, mouse, "Opponent");
        y += bf_h + 4.0f;
    }
    ...
    {
        float stack_h = 42.0f;
        DrawRectangle((int)px, (int)y, (int)pw, (int)stack_h,
                      (Color){215, 215, 170, 210});
        char stack_label[32];
        snprintf(stack_label, sizeof(stack_label), "Stack (%d)", gs->stack_size);
        DrawTextEx(g_font, stack_label,
                   (Vector2){px + 4.0f, y + 3.0f}, small_sz, 1.0f, DARKGRAY);
        float ex = px + 85.0f;
        for (int i = 0; i < gs->stack_size && i < MAX_STACK_DISPLAY; i++) {
            const StackEntry *se = &gs->stack[i];
            const char *sname = (se->card_vocab_idx >= 0)
                ? gui_card_name(se->card_vocab_idx) : "?";
            char chip[80];
            snprintf(chip, sizeof(chip), "[%s - %s]",
                     sname, se->controller_is_self ? "You" : "Opp");
            float chip_w = MeasureTextEx(g_font, chip, small_sz, 1.0f).x + 8.0f;
            Color chip_col = se->controller_is_self
                ? (Color){90, 150, 240, 220}
                : (Color){240, 110, 90, 220};
            DrawRectangle((int)ex, (int)(y + 12.0f), (int)chip_w, 22,
                          chip_col);
            DrawTextEx(g_font, chip,
                       (Vector2){ex + 4.0f, y + 15.0f}, small_sz, 1.0f, WHITE);
            ex += chip_w + 4.0f;
        }
        y += stack_h + 4.0f;
    }
    ...
    {
        float bf_h = 175.0f;
        render_battlefield(px, y, pw, bf_h, gs->self_permanents,
                           MAX_BATTLEFIELD_SLOTS, mouse, "You");
        y += bf_h + 4.0f;
    }
    ...
    {
        float hand_h = 85.0f;
        DrawRectangle((int)px, (int)y, (int)pw, (int)hand_h,
                      (Color){200, 225, 200, 190});
        ...
        for (int i = 0; i < MAX_HAND_SLOTS; i++) {
            ...
            if (vi >= 0) {
                ...
                DrawRectangle((int)(hx + 2.0f), (int)(hy + 2.0f),
                              (int)(card_w - 4.0f), (int)(hand_h - 4.0f),
                              (Color){195, 225, 195, 255});
                DrawRectangleLinesEx(
                    (Rectangle){hx + 2.0f, hy + 2.0f, card_w - 4.0f, hand_h - 4.0f},
                    1.0f, DARKGREEN);

                DrawTextEx(g_font, cname,
                           (Vector2){hx + 4.0f, hy + 4.0f}, small_sz, 1.0f, BLACK);
                DrawTextEx(g_font, ctype,
                           (Vector2){hx + 4.0f, hy + 16.0f}, 9.0f, 1.0f, DARKGRAY);
                // Oracle text: first ~3 lines
                BeginScissorMode((int)(hx + 2.0f), (int)(hy + 26.0f),
                                 (int)(card_w - 4.0f), (int)(hand_h - 30.0f));
                draw_wrapped_text(coracle, hx + 4.0f, hy + 27.0f,
                                  card_w - 8.0f, 8.5f, DARKGRAY);
                EndScissorMode();

                // Hover check for tooltip (hand cards use base stats)
                if (mouse.x >= hx && mouse.x < hx + card_w &&
                    mouse.y >= hy && mouse.y < hy + hand_h) {
    ...
    // ── 8. Self info bar ────────────────────────────────────────────────
    {
        ...
        snprintf(bar, sizeof(bar),
                 "YOU  Life:%d  Poison:%d  W:%d U:%d B:%d R:%d G:%d C:%d  Lands:%d  Lib:%d  GY:%d",
                 gs->self.life, gs->self.poison_counters,
                 gs->self.mana[0], gs->self.mana[1], gs->self.mana[2],
                 gs->self.mana[3], gs->self.mana[4], gs->self.mana[5],
                 gs->self.lands_played_this_turn,
                 gs->self_library_ct, self_gy);
    ...
    // ── 9. Graveyard / exile summary ────────────────────────────────────
    {
        ...
        for (int i = 0; i < MAX_GY_SLOTS; i++) {
            if (gs->self_graveyard[i] >= 0) {
                ...
                strncat(line, gui_card_name(gs->self_graveyard[i]),
        ...
        for (int i = 0; i < MAX_GY_SLOTS; i++) {
            if (gs->opp_graveyard[i] >= 0) {
                strncat(line, " ", sizeof(line) - strlen(line) - 1);
                strncat(line, gui_card_name(gs->opp_graveyard[i]),
                        sizeof(line) - strlen(line) - 1);
                strncat(line, ",", sizeof(line) - strlen(line) - 1);
            }
        }
    ...
    if (q_held && gs_hover_vocab_idx >= 0) {
        ...
        // Clamp to screen
        if (tp_x + tp_w > (float)SCREEN_WIDTH - 5.0f)
            tp_x = gs_hover_tx - tp_w - 14.0f;
        if (tp_y < 5.0f)                   tp_y = 5.0f;
        if (tp_y + tp_h > (float)SCREEN_HEIGHT - 5.0f)
            tp_y = (float)SCREEN_HEIGHT - tp_h - 5.0f;

        DrawRectangle((int)tp_x, (int)tp_y, (int)tp_w, (int)tp_h,
                      (Color){252, 248, 218, 252});
        DrawRectangleLinesEx(
            (Rectangle){tp_x, tp_y, tp_w, tp_h}, 2.0f,
            (Color){100, 80, 20, 255});

        float ty = tp_y + 6.0f;
        DrawTextEx(g_font, tname,
                   (Vector2){tp_x + 6.0f, ty}, 13.0f, 1.0f, BLACK);
        ty += 16.0f;
        DrawTextEx(g_font, ttype,
                   (Vector2){tp_x + 6.0f, ty}, small_sz, 1.0f, DARKGRAY);
        ty += 14.0f;
        ...
        if (show_pt) {
            char pt[40];
            if (disp_d > 0)
                snprintf(pt, sizeof(pt), "%d/%d  (%d damage)",
                         disp_p, disp_t, disp_d);
            else
                snprintf(pt, sizeof(pt), "%d/%d", disp_p, disp_t);
            DrawTextEx(g_font, pt, (Vector2){tp_x + 6.0f, ty},
                       small_sz, 1.0f, BLACK);
            ty += 14.0f;
        }

        // Oracle text clipped to tooltip
        BeginScissorMode((int)(tp_x + 4.0f), (int)ty,
                         (int)(tp_w - 8.0f),
                         (int)(tp_y + tp_h - ty - 4.0f));
        draw_wrapped_text(toracle, tp_x + 6.0f, ty,
                          tp_w - 12.0f, small_sz, BLACK);
        EndScissorMode();
    }
}
