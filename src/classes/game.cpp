...
//outer loop to assign each deck to proper player
    for (size_t i = 0; i < 2; i++) {
        ...
        //loop through each card and create an entity in appropriate library
        for (auto &&card_name : deck_a.main_deck) {
            for (size_t i = 0; i < card_name.first; i++) {
                ...
                coordinator.AddComponent(card_id, Zone(Zone::LIBRARY));
                coordinator.AddComponent(card_id, Owner(PLAYER_A));
            }
        }
    }
}
