...
name), _STEP_NAMES (ordered step display names), ACTION_CATEGORY_MAX,
_REF_NAMES / REF_ZONE_MAX (per-action zone_ref block), _MC_NAMES /
N_MANDATORY_CHOICES (the MandatoryChoice one-hot in the global-extras block),
and _OBS_KEYWORDS / N_OBS_KEYWORDS (the per-permanent keyword multi-hot order).
...
def parse_mandatory_choices():
    ...
    body = _enum_body(GAME_H, r"enum\s+MandatoryChoice")
    names = [tok.strip() for tok in body.split(",")]
    names = [n for n in names if re.fullmatch(r"[A-Z_][A-Z0-9_]*", n)]
    if not names or names[0] != "NONE":
        raise RuntimeError("MandatoryChoice parse failed (expected NONE first)")
    return names
