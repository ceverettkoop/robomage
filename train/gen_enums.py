...
_GAMESTATE_INTS = [
    ...
    "MAX_DELAYED_TRIGGER_SLOTS", "N_DELAYED_FIRE_KINDS",
]
...
# parse_int_constant — std::log1p is not a constant expression in C++17, so the C++
# side computes them at static-init from the very scales above. Python is emitted the
# SAME derivation over the SAME mirrored int, so neither side can end up dividing by a
# different number. (name, scale-constant-name) in emission order.
_MACHINE_LOG1P_DENOMS = [
    ("LOG_LIFE_DENOM", "LIFE_NORMALIZER"),
    ("LOG_LIBRARY_DENOM", "LIBRARY_NORMALIZER"),
]
...
def main():
    ...
    for name in _MACHINE_INTS:
        layout.append((name, parse_int_constant(MACHINE_IO_H, name),
                       "src/machine_io.h"))
    # Each log1p denominator is emitted as math.log1p(<scale>), so its scale must
    # itself be a mirrored int — otherwise the generated module would reference an
    # undefined name at import time instead of failing here.
    for _name, _scale in _MACHINE_LOG1P_DENOMS:
        if _scale not in _MACHINE_INTS:
            sys.exit(f"gen_enums.py: {_name} is derived from {_scale!r}, which is not "
                     f"in _MACHINE_INTS — add it there so it is mirrored too")
    ...
    for name, scale_name in _MACHINE_LOG1P_DENOMS:
        lines.append(f"{name} = math.log1p({scale_name})  # src/machine_io.h")
    lines += [
        "",
