...
# and which namespaces its checkpoints/recordings under a matching 'league/' subdir.
...
def _league_chunk(binary_path: str, learner_deck: str, roster: list[str],
    ...
    # that subdir under the checkpoint/recording dirs so snapshot saves don't fail.
    deck_subdir = os.path.dirname(learner_deck)
    if deck_subdir:
        os.makedirs(os.path.join(checkpoint_dir, deck_subdir), exist_ok=True)
        os.makedirs(os.path.join(RECORD_DIR, deck_subdir), exist_ok=True)
    vec_env = SubprocVecEnv([
