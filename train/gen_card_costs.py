...
def main():
    ...
    emit_cpp_header(cast_matrix, vocab_by_idx)


def emit_cpp_header(cast_matrix, vocab_by_idx):
    """Write src/gen/card_costs_gen.h with CARD_COST_MATRIX (from cast_matrix)
    and an all-zero CARD_ABILITY_COST_MATRIX, mirroring card_costs.py exactly."""
    def row_literal(row):
    ...
    for idx in range(N_TYPES):
        name = vocab_by_idx.get(idx, "")
        hdr.append(f"    {row_literal(zero_row)},  // {idx}: {name}")
    hdr += ["};", ""]
