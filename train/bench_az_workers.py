...
Defaults REPLICATE the az-league curriculum slots so the measured throughput is
the production workload: the EXHAUSTIVE-SELFPLAY matrix (every unordered roster
pair x --exhaustive-repeats + --scripted-cells rotating vs-scripted cells; the
slot index advances per leg exactly like consecutive league slots) at the shared
cli_spec training budget — every search/generation default (sims, worlds,
c_puct, td_n, the exploration clock, the playout cap, the opponent pool, the sb
knobs) is the az-* commands' own. Pass --random-draw for the mirror_frac-draw
schedule instead (then --games/--mirror-frac apply).
...
size, which is why ``--decks`` defaults to the whole league roster (multi-focus,
up to ~100 distinct groups on the 10-deck roster). Each leg's
...
runs AUTO batches (``--batches 0`` under az-train's own epoch_frac / q_mix
defaults) over a window defaulting to the bench legs' shards PLUS the most
...
alongside the fresh bench data; the stock az-train window would silently
...
Example (backgrounded so the log can be tailed):
    ...
    train/.venv/bin/python train/train.py bench-workers \
        --train > /tmp/bench_az_workers.log 2>&1 &
...
def schedule_preview(az_selfplay, args, focus, roster, seed, slot):
    ...
    if args.random_draw:
        ...
        sched, seats = az_selfplay.build_exhaustive_schedule_ex(
            ...
            repeats=args.exhaustive_repeats, scripted_cells=args.scripted_cells,
            slot=slot)
...
def run_leg(az_selfplay, args, focus, roster, workers, seed, slot, n_matches,
    ...
    res = az_selfplay.generate(
        ...
        exhaustive_repeats=args.exhaustive_repeats,
        scripted_cells=args.scripted_cells, slot=slot,
        full_search_frac=DEFAULT_AZ_FULL_SEARCH_FRAC,
        fast_sims=DEFAULT_AZ_FAST_SIMS, opp_pool_frac=DEFAULT_AZ_OPP_POOL_FRAC,
...
def final_train_eval(args, total_new_shards: int, pool_extra: int,
    ...
    if not args.no_eval:
        ...
        eval_cmd = [sys.executable, TRAIN_PY, "az-eval",
                    "--deck-a", args.deck_a, "--candidate", "gen",
                    "--gate-max-rounds", "1", "--games", str(args.eval_games)]
...
def run(args) -> None:
    """``train.py bench-workers`` entry."""
    import az_selfplay
    counts = parse_int_list(args.workers, "--workers")
    use_actor = az_selfplay._resolve_use_actor(args)
    ...
    mode = ("random-draw" if args.random_draw else
            f"exhaustive-selfplay x{args.exhaustive_repeats} + "
            f"{args.scripted_cells} scripted cells (curriculum-matching)")
    print(f"[bench-az] workers={counts} schedule={mode} "
...
if __name__ == "__main__":
    sys.exit("bench_az_workers.py was removed; use `train.py bench-workers` "
             "(--counts is now --workers)")
